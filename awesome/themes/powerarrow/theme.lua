...
theme.volume = lain.widget.pulse({
  timeout = 60, -- fallback polling (event-driven below handles most updates)
  cmd =
  "echo 'index: 0'; pactl get-sink-mute @DEFAULT_SINK@ | sed 's/Mute: /muted: /'; pactl get-sink-volume @DEFAULT_SINK@",
  settings = function()
    local vlevel = tonumber(volume_now.left) or 0
...
-- Event-driven volume updates via pactl subscribe
...
volicon:buttons(my_table.join(
  awful.button({}, 1, function() -- left click: mute
...
function theme.at_screen_connect(s)
  ...
  s.mywibox:setup {
    ...
    {             -- Right widgets
      ...
      arrow("#4B3B51", "#7493D2"),
      wibox.container.background(wibox.container.margin(wibox.widget { volicon, theme.volume.widget, layout = wibox.layout.align.horizontal }, dpi(3), dpi(3)), "#7493D2"),
      arrow("#7493D2", "#8DAA9A"),
      ...
      arrow("#8DAA9A", "#2F4F4F"),
      wibox.container.background(wibox.container.margin(wibox.widget { nil, neticon, net.widget, layout = wibox.layout.align.horizontal }, dpi(3), dpi(3)), "#2F4F4F"),
      arrow("#2F4F4F", "#777E76"),
